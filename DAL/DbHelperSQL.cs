...
namespace ServiceWeb.DAL
{
    ...
    {
        //数据库连接字符串(可以用web.config来配置)		
        //public static string connectionString = "server=192.168.1.93;database=ServiceWeb;uid=ServiceWeb;pwd=Hacker748!;Integrated security=true;";
        public static string connectionString = System.Configuration.ConfigurationManager.ConnectionStrings["strConnString"].ToString();
        public DbHelperSQL()
        {
        }
        ...
        public static DataSet GetPage(string sql, int currentPage, int pagesize, out int recordcount)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    command.Fill(ds, startRow, pagesize, "table");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    SqlCommand sqlcmd = new SqlCommand(sql, connection);
                     recordcount = int.Parse(sqlcmd.ExecuteScalar().ToString());
                    connection.Close();
                }
                catch (System.Data.SqlClient.SqlException ex)
                {
